...
def train(train_loader, teacher_model, model, optimizer, epoch, test_path, temperature = 2, alpha = 0.3):
    ...
    for i, pack in enumerate(train_loader, start=1):
        for rate in size_rates:
            ...
            # conv = nn.Conv2d(P1_tea.shape[-1], C, (3, 3))
            ...
            # P1_emb = log_softmax(P1_emb)
            # P1_emb = torch.nn.functional.softmax(P1_emb)
            # P1_emb = P1_emb.squeeze()
            # P1_emb = torch.mean(P1_emb.view(P1_emb.size(0), P1_emb.size(1), -1), dim=2)/temperature
            # print(P1_emb.shape)
            P1_tea_new = P1_tea
            ...
            P1_tea_emb_att = torch.nn.functional.scaled_dot_product_attention(P1_tea_emb, P1_tea_emb, P1_tea_emb)/temperature
            P1_emb_att = torch.nn.functional.scaled_dot_product_attention(P1_emb, P1_emb, P1_emb)/temperature
            
            # P1_tea_emb = log_softmax(P1_tea_emb)
            # P1_tea_emb = torch.nn.functional.softmax(P1_tea_emb)
            # P1_tea_emb = P1_tea_emb.squeeze()
            # P1_tea_emb = torch.mean(P1_tea_emb.view(P1_tea_emb.size(0), P1_tea_emb.size(1), -1), dim=2)/temperature
            # print(P1_tea_emb.shape)
            # P1_tea_emb = P1_tea_emb.unsqueeze(dim = 1)

            # P1_tea = torch.matmul(P1_tea, P1_tea_emb)  

            # P1_tea =  P1_tea + P1_tea_emb 
            # P1_tea = torch.nn.functional.softmax(P1_tea)/temperature
            # P2_tea = torch.nn.functional.softmax(P2_tea)/temperature
            
            P1_stu = torch.nn.functional.softmax(P1)/temperature
            P2_stu = torch.nn.functional.softmax(P2)/temperature

            # ---- loss function ----
            # cos_loss = kl_loss(P1_emb_att, P1_tea_emb_att)
